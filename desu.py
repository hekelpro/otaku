...
def search(query, num=0, DumpLink=[]):
	...
	for li in cek.find_all("li", {"style": "list-style:none;"}):
		...
		print("("+str(num)+") Judul: "+links.text)
...
def chek_support_dl(visit, resoVideo: int):
		...
		donlod = cok.find("div", {"class": "download"}).findAll("li")[resoVideo]
...
def eps_chekin(links, mahou=[]):
	...
	for eps in lii.find_all("li"):
		mahou.append(eps.find("a").get("href"))
	if len(mahou) == 0:
		print("\n[×] Tidak ada episode yang bisa di ambil")
		input("[=] Enter untuk kembali "); main_menu()
	else:
		print("\n[•] Episode berjumlah %s%s%s" % (col(92), str(len(mahou)), col(00)))
...
def main_menu():
	...
	if cari == False:
		...
		pilih = input("\n[?] Pilih pencarian: ")
		while int(pilih) > len(cari) or not pilih.isdigit() or pilih == "":
